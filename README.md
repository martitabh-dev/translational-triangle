...
python -m scripts.main --resume-forward results/papers_full.csv \
    ...
    --pubmed-api-key PUBMED_API_KEY \
    --mesh-descriptors dataset/desc2026.xml \
    --citations-details results/citations_details.csv \
    --output-dir results/
```
...
Unlike modes 1 and 2, `--citations-details` is **required** here: `papers_full.csv` is itself given as an explicit path in this mode, not necessarily one that lives inside `--output-dir`, so there is no folder to default `citations_details.csv` into.
...
| `--citations-details` | Path to `citations_details.csv` **Required** for mode 3 | `<output-dir>/citations_details.csv` |
...
- **`citations_details.csv`** — the detailed, row-by-row log behind every TD/TY/TC value in `papers_full.csv`: one row per *(original article) → (citing article found while exploring its forward-citation network)* relationship, generated by stage 3 (`forward_citations.py`) at the path given with `--citations-details` (see [Usage](#usage)). An original article can have anywhere from zero to dozens of rows here, one per citation actually explored across up to 4 generations. Columns:
